...
function cleanKey(key) {
    if (!key) return "";
    // Remove ALL whitespace, line breaks, and non-printable characters
...
function generateSignature(queryString, secret) {
    // PERCENT-ENCODE the payload before signing (required by Binance)
...
async function verifyBinanceKeys(apiKey, secretKey) {
    ...
    if (!cleanApi || !cleanSecret) throw new Error('Keys are empty after cleaning');
    
    const timestamp = await getServerTime();
    const queryString = `recvWindow=5000&timestamp=${timestamp}`;
    const signature = generateSignature(queryString, cleanSecret);
    const url = `https://api.binance.com/api/v3/account?${queryString}&signature=${signature}`;
    const response = await axios.get(url, {
        headers: { 'X-MBX-APIKEY': cleanApi },
        timeout: 10000
    });
    if (!response.data.canTrade) throw new Error('Spot & Margin Trading not enabled');
    return { success: true };
}
...
app.post('/api/set-api-keys', authenticate, async (req, res) => {
    ...
    try {
        await verifyBinanceKeys(cleanApi, cleanSecret);
        const users = readUsers();
        if (!users[req.user.email]) return res.status(404).json({ success: false });
        ...
        let msg = 'Invalid Binance API keys. ';
        if (err.message.includes('Spot & Margin Trading')) msg += 'Enable Spot & Margin Trading in Binance.';
        else if (err.message.includes('timestamp')) msg += 'Server time sync issue. Try again.';
        else if (err.message.includes('signature') || err.message.includes('Illegal characters')) msg += 'Secret key is incorrect or contains illegal characters.';
...
// ==================== BINANCE API WITH FIXED SIGNATURE ====================
...
app.get('/api/admin/users', authenticate, (req, res) => {
    ...
    const list = Object.keys(users).map(email => ({
        email, hasApiKeys: !!users[email].apiKey, isOwner: users[email].isOwner, isApproved: users[email].isApproved
    }));
...
app.listen(PORT, '0.0.0.0', () => {
    console.log(`\n🌙 Halal AI Trading Bot - FULLY FIXED VERSION`);
    console.log(`✅ Server running on port: ${PORT}`);
    console.log(`✅ Owner email: mujtabahatif@gmail.com | Password: Mujtabah@2598`);
    console.log(`✅ API key cleaning: enabled`);
    console.log(`✅ Signature encoding: percent-encoding enabled`);
